...
while True:
    try: 
        ...
        cv2.putText(frame, text, (10, 60), cv2.FONT_HERSHEY_DUPLEX, 1.6, (147, 58, 31), 2)
        ...
        if horizontal_ratio is not None and vertical_ratio is not None:
            mimicked_x = math.floor(600 * horizontal_ratio)
            mimicked_y = math.floor(400 * vertical_ratio)
        ...
        cv2.imshow("Cheating Classification Demo", frame)
        if cv2.waitKey(1) == 27:
            break

    except KeyboardInterrupt:
        pd.DataFrame({'ver':ver , 'hor' : hor , 'y' : y, 'test_x': x_test , 'test_y' : y_test}).to_csv(r"C:\Users\2018A00585\SW_-hackathon1\ML/data/final_test_2020.12.04.csv")
